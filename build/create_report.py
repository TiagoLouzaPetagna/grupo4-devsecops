...
add_para("O Semgrep foi executado primeiro com a configuração geral da comunidade, sem regra própria. O scan percorreu 1.027 arquivos, aplicou 408 regras e reportou 69 achados. A análise encontrou SQL Injection nas rotas de login e busca. Para a demonstração red e green, uma regra local mais estreita foi mantida apenas para garantir tempo e resultado determinísticos: o código vulnerável produziu um HIGH, a consulta foi parametrizada com replacements do Sequelize e o mesmo teste passou sem achados.")
...
add_heading("Regra local do gate", 3)
add_para("A regra em security/semgrep.yml não foi necessária para encontrar SQL Injection no scan geral. Ela foi mantida para o laboratório de 12 minutos, porque limita o escopo a uma construção conhecida e torna o red e green reproduzível. A metadata devsecops_severity normaliza o achado como HIGH e CWE-89. Em produção, regras locais devem ter testes positivos e negativos, responsável e revisão quando o framework mudar.")
...
add_table(["Cenário", "Arquivos", "Regras", "Achados", "Decisão"], [["Red", "1", "3", "1 HIGH", "Gate bloqueado"], ["Green", "1", "3", "0", "Gate aprovado"]], widths=[3.2, 2.2, 2.2, 3.0, 5.4], font_size=9.3, first_col_bold=True)
...
add_heading("Laboratório red green e DAST", 2)
add_code(".\\scripts\\run-lab.ps1 -Mode sast-red\n.\\scripts\\run-lab.ps1 -Mode sast-green\n.\\scripts\\run-lab.ps1 -Mode dast")
add_heading("Execuções complementares", 2)
add_code("# Semgrep geral\ndocker run --rm -v \"${ProjectDir}target/juice-shop:/src/target/juice-shop\" `\n  semgrep/semgrep:1.172.0 semgrep scan --config auto /src/target/juice-shop\n\n# Checkov\ndocker run --rm -v \"${ProjectDir}target/juice-shop/terraform:/tf:ro\" `\n  ghcr.io/bridgecrewio/checkov:3.3.11 -d /tf --framework terraform --compact")
